...
infile = '../data/aris_crap4_mass106_5mswc_2018-05-07_200ms.lmf'
...
def read_lmf(fileName):
    ...
    print( LM64NumberOfEvents )
    
    for event in range(0, LM64NumberOfEvents):

        # print( dataBlock[ 0 : 64 ] )
        # sys.exit(0)
        ...
        times.extend( [ 0, TDC[0], TDC[1], TDC[2], TDC[3], TDC[6] ] )
            
    ##END Loop through all events in the file

    ##Output time information 
    print( "Start:         (%d)" %LMStartTime, " ",
           time.strftime('%H:%M:%S %Y-%m-%d', (time.localtime(LMStartTime))) )

    print( "Stop:          (%d)" %LMStopTime, " ",
           time.strftime('%H:%M:%S %Y-%m-%d', (time.localtime(LMStopTime))) )
...
data, a, b = read_lmf( infile ) 

# print( data ) 

# outfile_tdc_data = '../data/sample_tdc_data.tsv'
outfile_processed_data = '../data/sample_processed_data.tsv'
# outfile_channels = '../data/sample_tdc_channels'

outfile_times = '../data/sample_tdc_times.txt'

times = np.array( times, dtype = int ) 

np.savetxt( outfile_processed_data, data, delimiter = '\t' )
np.savetxt( outfile_times, times, fmt="%d", delimiter = '\t' )
